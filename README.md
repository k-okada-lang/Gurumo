...
和食・イタリアン・フレンチ・焼肉・寿司・中華・カフェ・居酒屋など、飲食店を検索できるお客様向けの紹介サイトです。
キーワード／都道府県／エリア／業態の4項目検索、業態・人気エリアのアイコン検索、現在地からの距離順検索、店舗詳細（写真・営業時間・SNSリンク・地図）、店舗情報を編集できる管理（アドミン）画面を備えています。
...
├── ogp.png      SNSシェア時に表示されるOGP画像（任意。なくてもサイトは動作します）
└── README.md    このファイル
```

HTML・CSS・JavaScriptをすべて `index.html` 1ファイルにまとめてあります。他のファイルへの参照がないため、この1ファイルだけをコピーすればどこでもそのまま動きます。`ogp.png` はSNSでシェアされた時のプレビュー画像で、`index.html` と同じ場所に置くことで表示されます（置かなくてもサイト自体は問題なく動作します）。
...
- **検索**：キーワード／都道府県／エリア／業態の4項目で絞り込み、ご予算・新着・店名で並び替え。都道府県を選択すると、エリアの選択肢がその都道府県に登録されているエリアだけに自動的に絞り込まれます（例：「宮城県」を選ぶと、宮城県内に登録した市区町村だけがエリアの選択肢に表示されます）
- **業態アイコン検索**：和食／イタリアン／フレンチ／焼肉／寿司／中華／カフェ／居酒屋／その他をタップで絞り込み
- **人気エリアランキング検索**：アドミンで登録・並び替えしたエリアが、都道府県ごとにグループ分けされた上で、掲載店舗数つきのランキング形式で表示されます（初期値：東京都　銀座・渋谷・恵比寿・代官山・新宿・浅草・六本木・中目黒・池袋）。エリアをタップすると、検索欄の都道府県・エリアも連動して選択されます
- **現在地から探す**：ブラウザの位置情報を使って現在地から近い順に表示（店舗の緯度経度が未設定の場合はエリア名から自動推定）
- **店舗詳細ページ**：写真ギャラリー（最大3枚、サムネイルをクリックしてメイン写真を切り替え）、Instagram／TikTokアイコンリンク、住所（地図リンク付き）・営業時間・定休日、詳細文、予約・問い合わせボタン（電話／LINE／メール）
- **写真アップロード（アドミン）**：店舗ごとに最大3枚まで写真を登録可能。クリックでファイル選択、またはドラッグ＆ドロップでアップロードできます（1枚目が一覧・詳細ページのメイン写真になります）
- **店舗詳細文への写真の挿入（アドミン）**：店舗編集フォームの「店舗詳細文」は、文章の途中にカーソルを置いて「🖼 画像」ボタンを押すと、その位置に写真を挿入できるリッチテキスト入力欄になっています（太字にも対応）。店舗入り口・商品・店長の写真などを文章の間に差し込んで紹介文を作れます。以前にプレーンテキストで保存されていた説明文も、読み込み時に自動的に段落として扱われるため見た目は変わりません。
- **アドミン画面**：右上「店舗管理」から管理画面に入れます。画面は「掲載店舗管理」「設定」の2つのタブに分かれています
  - **掲載店舗管理タブ**：店舗の追加・編集・削除（都道府県・エリア、業態、写真、住所、営業時間、定休日、SNS・LINEリンク、詳細文、タグ、緯度経度など全項目を編集可能）。都道府県を選ぶとエリアの選択肢が連動して絞り込まれます
  - **設定タブ**：サイト内の文言・業態・人気エリア・通常エリア・よく使うタグの管理をまとめて行えます（詳細は下記）
- **サイト内の文言の設定（アドミン／設定タブ）**：サイト名・サブテキスト、トップページの見出し（小見出し／メイン見出し2行／説明文）、「業態から探す」「人気エリアから探す」の見出し、フッター文言を管理画面からまとめて変更できます。保存するとサイト全体・ブラウザタブのタイトルに即反映され、「初期値に戻す」でいつでも元の文言に戻せます
- **業態の管理（アドミン／設定タブ）**：業態の追加・名称変更（✎）・削除（×）が可能。業態ごとにアイコン（🍣🥩🍜など32種類の絵文字から選択）を設定でき、追加時・編集時ともにアイコン部分をクリックするとアイコン選択パネルが開きます。名称やアイコンを変更すると、その業態を使っている店舗にも自動的に反映されます。削除しても既存店舗の業態表示は残りますが、絞り込みの選択肢からは消えます
- **人気エリアの管理（アドミン／設定タブ）**：エリアを追加する際に都道府県を選択します（例：宮城県を選んで「仙台」を追加）。一覧は都道府県ごとにグループ分けして表示され、↑↓ボタンで同じ都道府県内での表示順位（ランキング順）を自由に並び替えられます（異なる都道府県のエリアとは順位が混ざりません）。各エリアの件数はその時点の登録店舗数から自動計算されるため、手入力の必要はありません。追加・並び替えしたエリアはホーム画面のランキング表示（都道府県ごとにグループ表示）と、店舗登録時のエリア選択リストの両方に自動反映されます
- **通常エリアの管理（アドミン／設定タブ）**：ホーム画面の「人気エリアから探す」ランキングには表示させたくないが、店舗登録や検索の絞り込みには使いたいエリアを、都道府県を選んで追加できます。一覧は都道府県ごとにグループ分けして表示されます。追加・削除のみのシンプルな管理で、順位はありません。店舗登録時のエリア選択リストには人気エリアと合わせて表示され、そのエリアの店舗を検索すると通常どおり検索結果に表示されます
- **店舗登録の都道府県・エリア選択**：まず都道府県を選び、エリアは自由入力ではなく、その都道府県に登録済みの人気エリア・通常エリアからスクロールして選ぶ方式です（アドミンで管理しているエリアと必ず紐付く仕組みです）
- **よく使うタグの管理（アドミン／設定タブ）**：「テラス席あり」「デートに人気」「ワイン豊富」など、よく使うタグの候補をアドミンで追加・削除可能。店舗編集フォームでは候補をクリックするだけでタグの追加・削除ができます（自由入力欄も引き続き利用できます）
- **LINEで問い合わせるボタン**：店舗編集フォームで「LINE 相談先URL」を入力すると、詳細ページの「電話で予約する」ボタンの下に自動的に表示されます（未入力の店舗には表示されません）
- **ロゴ・ファビコン**：ヘッダーの「GURUMO」文字の左に、皿の中でフォークとナイフが交差するオリジナルアイコン（テラコッタ→オリーブの縦グラデーション）を配置。同じアイコンをブラウザタブのファビコンにも使用しています
- **掲載プラン（プラチナ／ゴールド）**：詳しくは下記の「掲載プラン（プラチナ／ゴールド契約）について」を参照してください
- **店舗詳細ページのタブ切替（基本情報／メニュー／内観）**：詳しくは下記の「店舗詳細ページのタブ（基本情報／メニュー／内観）について」を参照してください
- **ブログ／コラム／NEWS機能**：詳しくは下記の「ブログ・コラム・NEWS機能について」を参照してください
- **SEO対策**：詳しくは下記の「SEO対策について」を参照してください

## 掲載プラン（プラチナ／ゴールド契約）について

加盟店ごとに「掲載プラン」を設定できる機能です。アドミン画面の店舗編集フォームで「掲載プラン」欄からプラチナ・ゴールド・なし（通常）を選択します。

- **プラチナプラン**：ホーム画面の「★ おすすめの店舗」セクション（検索結果より上、プラチナ店舗が1件もない場合はセクションごと非表示）に表示されるほか、検索結果でも常に一番上に表示されます。
- **ゴールドプラン**：検索結果で、プラチナ店舗の下・通常店舗の上に表示されます。
- **表示順位（ランク）**：プラチナ・ゴールドそれぞれのプラン内で1位、2位…と表示順位を選べます。同じプラン内で他の店舗がすでに使っている順位は選択肢に出てこない仕組みになっており、順位の重複は起こりません（保存時にも重複がないか二重チェックしています）。プランを「なし」に戻すと、その順位は他の店舗が使えるようになります。
- **検索結果内の並び順**：常に「プラチナ（ランク順）→ ゴールド（ランク順）→ それ以外（選択した並び替え条件）」の順になります。絞り込み条件に合致した店舗だけが対象になるため、検索条件に合わないプラチナ／ゴールド店舗が優先表示されることはありません。
- **バッジ表示**：検索結果カード・店舗詳細ページに、プラチナ／ゴールドのバッジが自動的に表示されます。

## 店舗詳細ページのタブ（基本情報／メニュー／内観）について

店舗詳細ページに「基本情報」「メニュー」「内観」の切替タブを設けています。

- **基本情報タブ**：従来どおり、写真ギャラリー・住所・営業時間・SNSリンク・詳細文などを表示します。
- **メニュータブ**：アドミンの店舗編集フォーム内「メニュー」欄で、料理名・価格・説明文を自由に追加・削除できます。登録した順番で一覧表示されます。メニューが未登録の店舗は「準備中」と表示されます。
- **内観タブ**：「テーブル席」「カウンター席」「個室」の3カテゴリーに分けて内観写真を管理できます（各カテゴリー最大4枚、クリックまたはドラッグ＆ドロップでアップロード）。カテゴリーごとに写真が未登録の場合は「準備中」と表示されます。

いずれもアドミンの店舗編集フォームから追加・編集でき、保存すると店舗詳細ページに即時反映されます。

## ブログ・コラム・NEWS機能について

サイト全体で使えるお知らせ・コラム・NEWS記事の投稿機能です。ヘッダーの「お知らせ・コラム」から記事一覧ページ（`#/blog`）に、各記事は個別URL（`#/blog/記事のスラッグ`）で表示されます。

- **アドミンでの記事管理**：管理画面に「ブログ管理」タブを追加しました。新規作成・編集・削除ができます。
- **リッチテキストエディタ**：本文はツールバー付きのエディタで編集します。見出し（H1・H2・H3）、太字、画像の挿入、内部リンク（例：店舗詳細ページへのリンク `#/store/店舗ID` や他記事へのリンク）、ボタン（ラベルとリンク先を指定してCTAボタンを設置）に対応しています。なお、記事タイトルは自動的にページのH1として出力されるため、本文内のH1は多用せず、通常はH2から使うことを推奨します（ツールバーにもその旨のヒントを表示しています）。
- **SEO設定**：記事ごとにSEOタイトル・meta description・URLスラッグを個別に設定できます（スラッグは記事タイトルから自動生成されますが、手動で変更も可能。他の記事と重複する場合は自動的に末尾に連番が付きます）。公開中の記事には構造化データ（JSON-LD：BlogPosting）も自動的に埋め込まれます。
- **公開ステータス**：「下書き」「公開」「予約投稿」の3種類から選べます。
  - 下書き：アドミン画面のみで確認でき、サイトの一覧・詳細ページには表示されません。
  - 公開：即座に一覧・詳細ページに表示されます。
  - 予約投稿：公開日時を指定すると、その日時を過ぎてから一覧・詳細ページに表示されます。

### 予約投稿についての注意点（重要）

このサイトはサーバーを持たない構成のため、予約投稿の公開判定は「誰かがページを開いたタイミングで、指定した公開日時をすでに過ぎているかどうか」をブラウザ側で毎回チェックする方式です。裏側でサーバーが自動的に記事を公開する仕組みではないため、指定した日時ちょうどに自動公開されるわけではなく、**その日時を過ぎたあとに誰かが（アクセスした人が）サイトを開いた時点で、その人の画面に公開状態として表示される**という動作になります。日時を過ぎるまでは下書きと同様、サイトには表示されません。

## SEO対策について

このサイトには次のSEO対策を組み込んでいます。

- **店舗ごとの個別URL**：店舗詳細ページを開くとURLが `index.html#/store/店舗ID` に変わります。このURLをそのままブックマーク・SNS投稿・LINEなどで共有すると、開いた人はその店舗の詳細ページに直接アクセスできます。ブラウザの「戻る」「進む」ボタンにも対応しています。
- **ページごとに自動で切り替わるタイトル・meta description**：店舗詳細ページを開くと、ブラウザのタブタイトルとmeta descriptionが「店舗名（都道府県エリア・業態）｜サイト名」とその店舗のキャッチコピーに自動的に切り替わります。管理画面（アドミン）はmeta robotsを`noindex, nofollow`にして検索結果に出にくくしています。
- **構造化データ（JSON-LD）**：サイト全体にWebSiteの構造化データを、店舗詳細ページには店舗名・住所（`addressRegion`に都道府県を設定）・業態・電話番号・写真などを含むRestaurantの構造化データを自動的に埋め込みます。Googleなどがリッチリザルト（検索結果の強調表示）に利用できる情報です。
- **OGP／Twitter Card**：SNSでシェアされた際のタイトル・説明文・画像（`ogp.png`）を設定しています。
- **パンくずリスト（構造化データ対応）**：店舗詳細ページに「GURUMO ＞ 業態 ＞ 店舗名」、ブログ一覧ページに「GURUMO ＞ お知らせ・コラム」、ブログ詳細ページに「GURUMO ＞ お知らせ・コラム ＞ 記事タイトル」というパンくずリストを画面上に表示するとともに、同じ内容をBreadcrumbList構造化データ（JSON-LD）としても自動的に出力します。パンくずの「業態」部分をクリックすると、ホーム画面に戻ってその業態で絞り込んだ検索結果が表示されます。
- **SEO設定（アドミン／設定タブ）**：管理画面の「設定」タブに「SEO設定（検索エンジン対策）」カードがあり、以下を管理できます。
  - **meta description**：検索結果・SNSシェア時の説明文（トップページ用。店舗詳細ページはその店舗のキャッチコピーが優先されます）
  - **サイトの公開URL**：GitHub Pages等で実際に公開したURLを登録すると、canonicalタグ・OGPのURL・店舗詳細ページの個別URル・サイトマップに反映されます
  - **sitemap.xml／robots.txtのダウンロード**：現在の設定内容から生成してダウンロードできます。ダウンロードした2つのファイルは `index.html` と同じ場所（GitHub Pagesならリポジトリ直下）に置いてください

### 知っておいていただきたい制限事項

このサイトはサーバーを持たない「1枚のHTMLファイル＋ブラウザ内で描画」という構成のため、SEO対策にも以下の限界があります。

- **店舗データの保存場所による違い**：GitHub Pagesなどスタンドアロンで公開した場合、店舗データはアクセスした人のブラウザの`localStorage`に保存されます（README内「データの保存について」参照）。そのため、検索エンジンのクローラーが最初にサイトを訪れた際に認識できる店舗情報は、`index.html`に埋め込まれている初期データ（デフォルトの8店舗）のみです。アドミン画面から追加した店舗は、それを追加した人のブラウザにしか保存されないため、そのままでは検索エンジンには基本的に認識されません。全員に同じ店舗データを検索エンジンにも認識してもらいたい場合は、店舗データを外部のデータベース・Headless CMSなどに置き換える改修が必要です（README内「データの保存について」を参照）。
- **URLの`#`（ハッシュ）部分について**：店舗ごとのURL（`#/store/店舗ID`）はブラウザ内だけで処理される仕組み（ハッシュルーティング）で、サーバーの設定が一切不要な代わりに、Googleなどの検索エンジンが個々の店舗ページを「別々の独立したページ」として確実にインデックスしてくれる保証はありません（Google公式には、より確実な方法としてサーバー側で個別のURLを用意する構成が推奨されています）。共有・ブックマーク用のURLとしては問題なく機能しますが、検索結果に店舗ごとのページとして安定的に表示させたい場合は、店舗ごとに実体のあるHTMLファイルを生成するなど、サーバー側での対応が必要になります。
- 以上により、今の構成でできるのは「基本的なメタ情報・構造化データの整備」「共有しやすい個別URL」までで、店舗数が多い本格的な集客サイトとして検索順位を本気で狙う場合は、バックエンド（データベース＋サーバーサイドでのページ生成）を持つ構成への移行をおすすめします。
...
アップロードした写真は、ブラウザ内で自動的に縮小・圧縮した上でデータの中に直接埋め込んで保存しています（外部の画像サーバーは使用していません）。`localStorage` は一般的に1ドメインあたり5〜10MB程度までという制限があるため、写真を多くの店舗に大量に登録すると保存できなくなる場合があります（保存に失敗した場合は画面に通知が表示されます）。多数の店舗・高画質の写真を扱う本格運用では、画像を外部のストレージ（例：Cloudinary、S3、Firebase Storageなど）にアップロードし、そのURLだけを保存する方式への改修をおすすめします。

## カスタマイズのヒント

- 店舗の初期データは `index.html` 内の `<script>` にある `DEFAULT_STORES` 配列にあります（`photos` 配列に写真のURL／データを最大3件まで、`prefecture` に都道府県名を格納します）
- 写真の最大枚数や縮小サイズは `index.html` 内の `processPhotoFile` 関数内（`maxDim`・JPEG品質の値）で調整できます
- 配色・フォントは `index.html` 内の `<style>` 冒頭の `:root { --bg: ...; --terracotta: ...; }` などの変数から調整できます
- 業態の初期値とアイコンは `index.html` 内の `DEFAULT_GENRES` 配列（`{name, icon}` の形）にあります。アイコンは🍣🍜🍕のような絵文字（32種類）から選ぶ方式で、絵柄自体は `ICON_LIBRARY` に、選択パネルに表示する順番は `ICON_PICKER_ORDER` にあります（公開後はアドミン画面から追加・変更・削除するのが基本です）
- 都道府県の一覧は `index.html` 内の `PREFECTURES` 配列（47都道府県の固定リスト）にあります。こちらはアドミン画面から追加・削除するものではない前提の定数です
- 人気エリアの初期値は `index.html` 内の `DEFAULT_AREAS` 配列に、通常エリアの初期値は `DEFAULT_NORMAL_AREAS` 配列にあります（いずれも `{name, prefecture}` の形。公開後はアドミン画面から追加・削除するのが基本です）
- よく使うタグの初期値は `index.html` 内の `DEFAULT_TAG_PRESETS` 配列にあります（公開後はアドミン画面から追加・削除できます）
- 予約ボタンは、電話番号が登録されていれば「電話で予約する」（tel:リンク）、なければメールでの問い合わせ（mailto:リンク）に自動的に切り替わります。LINE URLが登録されている場合は、その下に「LINEで問い合わせる」ボタンが追加で表示されます
- ロゴアイコンは `index.html` 内の `.brand-icon` （ヘッダー）と、`<head>` 内の `<link rel="icon">` （ファビコン）の両方に同じSVGが埋め込まれています。デザインを変更する場合は、この2箇所のSVG（円のリング＋回転させたフォーク・ナイフのパス）とタイルの背景色（`linear-gradient(180deg, var(--terracotta), var(--olive))`）を書き換えてください
- meta description・サイトの公開URLの初期値は `index.html` 内の `DEFAULT_SITE_TEXT` の `metaDescription` ・ `siteUrl` にあります（公開後はアドミン画面の「設定」タブ→「SEO設定」から変更するのが基本です）
- OGP画像（`ogp.png`）を差し替えたい場合は、同じファイル名で `index.html` と同じ場所に置き換えてください。サイズは1200×630px程度を推奨します
- 内観写真の最大枚数・縮小サイズは `index.html` 内の `processInteriorPhotoFile` 関数（`maxDim`・JPEG品質の値）で、ブログ記事のアイキャッチ画像・本文内画像の縮小サイズはそれぞれ `processPostHeroFile`・`processPostContentImageFile` 関数で調整できます。店舗詳細文に挿入する画像の縮小サイズは `processStoreDescImageFile` 関数で調整できます
- ブログ記事の初期データは `index.html` 内の `DEFAULT_POSTS` 配列にあります（公開後はアドミン画面の「ブログ管理」タブから追加・編集するのが基本です）
